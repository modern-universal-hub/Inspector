...
// holds the most recently analyzed source, so the download button always
// has something current to package up
let lastAnalyzed = { html: '', css: '', js: '', label: '' };

function renderCodeWithLines(elId, code){
  const el = document.getElementById(elId);
  const lines = code.split('\n');
  el.innerHTML = lines.map((line, i) =>
    `<span class="code-line"><span class="ln">${i + 1}</span><span>${escapeHtml(line) || ' '}</span></span>`
  ).join('');
}
...
function analyzeSource(html, sourceLabel){
  ...
  const cssOut = (externalCss.length ? `/* external stylesheets:\n${externalCss.join('\n')}\n*/\n\n` : '') + (styleTags || '/* no inline <style> found */');
  const jsOut = (externalJs.length ? `// external scripts:\n// ${externalJs.join('\n// ')}\n\n` : '') + (scriptTags || '// no inline <script> found');

  document.getElementById('code-html').textContent = html;
  document.getElementById('code-css').textContent = cssOut;
  document.getElementById('code-js').textContent = jsOut;

  renderCodeWithLines('code-html', html);
  renderCodeWithLines('code-css', cssOut);
  renderCodeWithLines('code-js', jsOut);

  lastAnalyzed = { html, css: cssOut, js: jsOut, label: sourceLabel };
...
btnDownloadSource.addEventListener('click', async () => {
  if (!lastAnalyzed.html) {
    alert('Inspect a page first — there\'s nothing analyzed yet to download.');
    return;
  }

  const name = safeFilenameFrom(lastAnalyzed.label);
  btnDownloadSource.disabled = true;
  btnDownloadSource.textContent = 'Packaging…';

  try {
    if (typeof JSZip === 'undefined') throw new Error('JSZip not available');

    const zip = new JSZip();
    zip.file('index.html', lastAnalyzed.html);
    zip.file('extracted-styles.css', lastAnalyzed.css);
    zip.file('extracted-scripts.js', lastAnalyzed.js);
    zip.file('source.txt', `Fetched from: ${lastAnalyzed.label}\nDownloaded: ${new Date().toISOString()}`);
    ...
    // fall back to downloading the raw HTML alone if zipping fails for any reason
    const blob = new Blob([lastAnalyzed.html], { type: 'text/html' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `${name}.html`;
    a.click();
  } finally {
    btnDownloadSource.disabled = false;
    btnDownloadSource.textContent = 'Download source';
  }
});
